...
public class Anagram {
	...
	public static void main(String[] args) {
		...
		for (int t = 0; t < T; t++) {
			...
			int strLn = input.length();
			if (strLn % 2 == 1) {
				System.out.println("-1");
			} else {
				HashMap<Character, Integer> fseq = new HashMap<Character, Integer>();
				HashMap<Character, Integer> sseq = new HashMap<Character, Integer>();
				for (int i = 0; i < strLn; i++) {
					Character tchar = input.charAt(i);
					if (i < (strLn / 2)) {
						if (fseq.containsKey(tchar)) {
							fseq.put(tchar, fseq.get(tchar) + 1);
						} else {
							fseq.put(tchar, 1);
						}
					} else {
						if (sseq.containsKey(tchar)) {
							sseq.put(tchar, sseq.get(tchar) + 1);
						} else {
							sseq.put(tchar, 1);
						}
					}
				...
				int cnt = 0;
				for (Character tchar : fseq.keySet()) {
					int fcnt = fseq.get(tchar);
					if (sseq.containsKey(tchar)) {
						int scnt = sseq.get(tchar);
						if (scnt < fcnt) {
							cnt += (fcnt - scnt);
						}
					} else {
